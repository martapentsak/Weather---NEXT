...
export async function fetchWeeklyWeather(location: Location) {
  const {
...
async function fetchForecast(location: Location, days: number) {
...
export async function fetchTodayWeather(location: Location) {
  const {
    forecast: { forecastday },
    location: loc,
